...
{
// clang-format off
    constexpr char licenseKey[] = "";
// clang-format on
...
AppController::getImageTargetResult(VuMatrix44F& projectionMatrix, VuMatrix44F& modelViewMatrix, VuMatrix44F& scaledModelViewMatrix)
{
    ...
    if (result){
        return const_cast<char *>(targetName);
    }
    return "false";
}
...
AppController::createObservers()
{
    ...
    if (mTarget == IMAGE_TARGET_ID)
    {
        const char* target_names[2]
                = { "51705", "6490" };

        for (int i = 0; i < 2; i++) {
            auto imageTargetConfig = vuImageTargetConfigDefault();
            imageTargetConfig.databasePath = "QR_AR_DB_2.xml";
            imageTargetConfig.targetName = target_names[i];
